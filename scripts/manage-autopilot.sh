...
case "$action" in
  install)
    ...
    sed -e "s|/Users/user/Dev/Projects/job-agent|$REPO_DIR|g" "$REPO_DIR/launchd/$DISCOVER_PLIST" > "$LAUNCHD_DIR/$DISCOVER_PLIST"
    sed -e "s|/Users/user/Dev/Projects/job-agent|$REPO_DIR|g" "$REPO_DIR/launchd/$APPLY_PLIST" > "$LAUNCHD_DIR/$APPLY_PLIST"
