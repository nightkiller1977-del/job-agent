...
| `INDEED_EMAIL` / `INDEED_PASSWORD` | `indeed.py` | `_auto_login()` — fills form programmatically (`redacted@example.com`) |
| `USAJOBS_EMAIL` / `USAJOBS_PASSWORD` | `usajobs.py` | `_auto_login()` via login.gov; pauses for 2FA if triggered |
| `COMPANY_EMAIL` / `COMPANY_PASSWORD` | `jobright.py` `_company_portal_login()` | Primary ATS portal auto-login — `redacted@example.com` (Motorola, Booz Allen, Greenhouse, SAIC, PNC, Capital One, ManTech) |
| `COMPANY_EMAIL_ALT` / `COMPANY_PASSWORD_ALT` | `jobright.py` `_company_portal_login()` | Fallback ATS portal login — `redacted@example.com` (GDIT, government portals); tried automatically if primary login fails |
...
```bash
# crontab -e
0 8 * * 1-5 cd /Users/user/Dev/Projects/job-agent && source .venv/bin/activate && python src/main.py apply --auto-submit >> /tmp/job-agent.log 2>&1
```
